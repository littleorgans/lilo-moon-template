...
- Atlas SQL migrations own the database schema. `db/drizzle/_generated/schema.ts` is generated by
  ...
  Atlas migration changes. Applications import it as `@littleorgans/drizzle-schema` and query
  through typed Drizzle inside `withPrincipal`; raw `sql` is for what the builder cannot express.
...
in the same pull request. `root:skills-check` fails a cited path the tree lacks and anything the
agent-runtimes catalog would refuse. It cannot tell that a sentence became false. Cite a repository
