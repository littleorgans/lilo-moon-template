...
describe("loadConfig", () => {
  ...
  it("derives the same key from the same password, and a different one otherwise", () => {
    const same = loadConfig(complete).cookieKey;
    expect(loadConfig(complete).cookieKey.equals(same)).toBe(true);
    const changed = loadConfig({ ...complete, WORKOS_COOKIE_PASSWORD: "z".repeat(32) });
  ...
  it.each([
    ...
    expect(() => loadConfig({ ...complete, [name]: undefined })).toThrow(name);
  });

  it("names an empty value as missing, not as present", () => {
    expect(() => loadConfig({ ...complete, WORKOS_API_KEY: "" })).toThrow("WORKOS_API_KEY");
  });

  it("lists every missing name at once instead of one per run", () => {
    expect(() =>
      loadConfig({ ...complete, WORKOS_API_KEY: undefined, WORKOS_CLIENT_ID: undefined }),
  ...
  it("refuses a cookie password shorter than 32 characters", () => {
    expect(() => loadConfig({ ...complete, WORKOS_COOKIE_PASSWORD: "short" })).toThrow(
  ...
  it("turns Secure cookies off for an http redirect uri and on otherwise", () => {
    expect(loadConfig(complete).secureCookies).toBe(false);
    expect(
      loadConfig({ ...complete, WORKOS_REDIRECT_URI: "https://cubicell.dev/callback" })
        .secureCookies,
    ).toBe(true);
  });

  // Sign-in has to work before Postgres exists, or the template cannot be run at all until someone
  // provisions a database.
  it("treats an absent DATABASE_URL as a runnable state", () => {
    expect(loadConfig(complete).databaseUrl).toBeNull();
    expect(loadConfig({ ...complete, DATABASE_URL: "postgres://x" }).databaseUrl).toBe(
      "postgres://x",
    );
  });
});
