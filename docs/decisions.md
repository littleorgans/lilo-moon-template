...
`lib/tsserver.js`. The editor therefore uses `js/ts.tsdk.path` at `node_modules/typescript` with
`js/ts.experimental.useTsgo`, plus the TypeScript 7 extension.

`oxlint-tsgolint` encodes the TypeScript release it was built against as
`MAJOR.MINOR.(typescriptPatch * 1000 + tsgolintPatch)`. `7.0.2001` is TypeScript `7.0.2` with
tsgolint patch `1`. Two pins, one type system. If they diverge, lint and `tsc` disagree.
...
[Prove every gate](../AGENTS.md#prove-every-gate) exists for that reason. Add a deliberate
violation, record the failure, revert it, then trust the green run. PR #18 is the CI example: a
deliberate oxfmt violation failed, the revert passed.
...
does not prove the named behaviour exists. A test is proven when a wrong implementation fails it.
Replace the body with an identity, or a stub, and watch `moon run <project>:test` fail. Then put
the body back. Coverage thresholds that would make this cheaper are #5 and are unbuilt.

## Identity and entitlements

WorkOS AuthKit is the identity choice: users, organizations, roles, permissions, and Entitlements
sourced from Stripe, delivered as claims on the session JWT. Do not build an entitlements service,
a sync webhook, or a billing database to approximate that. Buy it. Reopen the question when Stripe
entitlements cannot express a rule the product actually has.

The application exemplar does not yet sign anyone in. That wiring is #16. Token verification from
a language other than JavaScript is #17. The adapter seam is #23.

When those land, verification uses the WorkOS JWKS URL and a stock JWT library. No vendor SDK on
the verification path. The mapped `Principal` is `userId`, `orgId`, `roles`, `permissions`, and
`entitlements`. WorkOS `role` is organization membership. It must not become `SET ROLE` in
Postgres.
...
defaults pulled as strings, partitioned parents dropped, and index opclasses lost. RLS policies, generated columns, and partial indexes are not a claimed
pull surface. Pull output is generated after Atlas apply. It is never a source, and it is never
fed back into a migration.
...
Do not send a WorkOS JWT to PostgREST. supabase/auth#2476 has been open since 2026-04-08. WorkOS
publishes JWKS at `api.workos.com/sso/jwks/<clientId>`. Supabase third party auth looks up
`{issuer}/.well-known/jwks.json`. Hosted PostgREST returns `PGRST301 JWSInvalidSignature`. RLS
still holds on a direct Postgres connection: verify the JWT in process, then
`set local role authenticated` and `set_config` for `request.jwt.claims` and
`request.jwt.claim.sub` so `auth.uid()` and `auth.jwt()` work. Re-evaluate PostgREST when that
issue closes.
...
layout, the generator shapes that exist, WorkOS for identity, Atlas for SQL, Drizzle as generated
output, Supabase as a host.

Not settled here: which application framework you keep, whether you publish, to which registry,
which license, which package scope, when the first Rust or Python member lands, and every product
decision above the baseline.
