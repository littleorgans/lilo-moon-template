...
## New projects come from a template generated from the reference

`@littleorgans/create-app` writes a new project. Its template is generated by its build from
`apps/web`, `services/api`, `db/`, the workspace root and `@littleorgans/db`'s migrations at the
same commit, and it is never committed. A hand-kept copy, or the copy list the adoption guides
carried before, drifts from the reference in the first change nobody mirrors. Generation leaves
only the rewrite rules to drift, so those fail loudly instead: every root entry, root task and
`.env.example` paragraph is classified, and every edit names the text it expects, so an
unclassified addition or a moved anchor stops the build. `root:published-shape` then runs the packed
command for a web app with a service and a database, a web app alone, and a service alone, each
installed from the packed tarballs, and requires the project's own `moon ci --force` to pass with
`moon sync` changing nothing. That run is what makes "cannot drift" a checked claim.

The build formats the template with the project's own formatter settings, so the first
`format-check` passes without a `format` step. Two results depend on the name a project chooses,
which the build cannot know: where a key sorts in `package.json`, and where an import sorts. The
command sorts dependency keys again after substituting names, and the build checks that its
`package.json` output is exactly what `JSON.stringify` prints. The generated `.oxfmtrc.json` puts
the project's own scope in the internal import group, so its imports no longer sort among the
`@littleorgans/*` ones.

The command writes files and prints steps; it installs nothing and commits nothing. Judgment
stays with the person or the skill driving it. The organization policy has no default. Names and
ports default to the reference's, and the output lists every default taken, with its flag. A
service implies the database because `loadServiceConfig` requires `DATABASE_URL`. It asks
questions only at a terminal, with Node's own `readline`, and has no runtime dependencies. It never
writes into a directory that holds anything but `.git`, so adding a part to an existing project
means creating a scratch project and moving the part across.
...
into the projects the packed `create-app` writes, and their typecheck, tests and lint read their
settings from there.
