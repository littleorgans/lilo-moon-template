...
- `apps/web/src/server/` is the composition root. `auth.ts` selects `GoogleOAuth`, `/app`,
  ...
  title and the sign-in and session-error text) and `SHOW_THEME_LAB`, true in development or when built with `VITE_ENABLE_THEME_LAB=true`.
  This module is public data imported into both bundles; keep credentials and service imports out.
  `startup.ts` is a Nitro plugin that runs `loadAuthConfig` before the standalone Node server listens, so a bad
  cookie password fails the deploy rather than the first request. Dev skips validation only when
  all auth values are absent or empty. Other presets need a separate startup/readiness probe.
...
and 400 for unsupported flows and ambiguous provider 4xx refusals. The default log sink writes one JSON line
